...
class WeChatVFSNative {
  ...
  static bool initialize() {
    ...
    try {
      ...
      _registerCallback = _lib!
          .lookup<NativeFunction<RegisterCallbackNative>>(
            'wechat_vfs_register_keys',
          )
          .asFunction();

      _unregisterCallback = _lib!
          .lookup<NativeFunction<UnregisterCallbackNative>>(
            'wechat_vfs_unregister_keys',
          )
          .asFunction();

      _getCallbackCount = _lib!
          .lookup<NativeFunction<GetCallbackCountNative>>(
            'wechat_vfs_get_callback_count',
          )
          .asFunction();
  ...
  static Future<bool> registerDatabaseKey(String dbPath, String hexKey) async {
    ...
    try {
      ...
      // 转换密钥为hex字符串
      String encKeyHex = encKey
          .map((b) => b.toRadixString(16).padLeft(2, '0'))
          .join('');
      String macKeyHex = macKey
          .map((b) => b.toRadixString(16).padLeft(2, '0'))
          .join('');
  ...
  static void _decryptCallback(
    Pointer<Utf8> dbPathPtr,
    int pageNum,
    Pointer<Uint8> encryptedPtr,
    Pointer<Uint8> decryptedPtr,
  ) {
    try {
  ...
  /// 派生加密密钥（PBKDF2）
  static Future<List<int>> _deriveEncryptionKey(
    List<int> key,
    List<int> salt,
  ) async {
    final pbkdf2 = Pbkdf2(
  ...
  /// 派生MAC密钥
  static Future<List<int>> _deriveMacKey(
    List<int> encKey,
    List<int> salt,
  ) async {
