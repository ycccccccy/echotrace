...
在打开软件之前，你需要准备好运行环境和解密密钥。

### 1.1 环境要求与密钥获取
解密微信数据库需要特定的 64 位密钥。

*   **前提条件**：你需要安装 **PC 微信 4.0.x** 版本（若版本过低请先升级）。
*   **获取工具**：[wx_key 获取工具 (Windows)](https://github.com/ycccccccy/wx_key/)
*   **操作步骤**：
    1.  登录 PC 微信。
    2.  运行 `wx_key` 工具。
    3.  复制显示的 **数据库密钥**（64 位十六进制）。
    4.  *(可选)* 建议同时复制 **图片解密密钥**（AES Key 和 XOR Key），以便后续解密图片。

![wx_key](img/wx_key.png)

### 1.2 下载 EchoTrace
*   **下载地址**：[EchoTrace Releases](https://github.com/ycccccccy/echotrace/releases)
*   **安装**：下载最新版本的 `.zip` 压缩包，解压后双击运行 `EchoTrace.exe` 即可，无需安装。

---

## 2. 第一步：基础配置

打开 EchoTrace，默认进入 **设置** 页面，请按顺序完成以下配置：

### 2.1 填写密钥
*   **解密密钥**：粘贴刚才获取的 64 位数据库密钥。
...
2.  **右侧设置参数**：
    ...
    *   **导出格式**：
        ...
        *   `Excel`：适合做表格统计。
        *   `JSON`：适合开发者进行二次开发或导入到其他应用中（例如 [ChatLab](https://chatlab.fun/) 等）。
